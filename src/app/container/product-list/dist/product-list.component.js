...
var ProductListComponent = /** @class */ (function () {
    function ProductListComponent() {
        ...
        this.products = [
            ...
            Number(b.is_in_inventory) - Number(a.is_in_inventory);
            if (Number(b.is_in_inventory) === Number(a.is_in_inventory)) {
                return a.name.localeCompare(b.name);
            }
            return Number(b.is_in_inventory) - Number(a.is_in_inventory);
        });
        this.allProducts = this.products.length;
        this.inStockProducts = this.products.filter(function (prod) { return prod.is_in_inventory; }).length;
        this.outOfStockProducts = this.allProducts - this.inStockProducts;
    }
    ProductListComponent.prototype.EventRaised = function (event) {
        if (event === 'All') {
            this.selectedRadioBtn = 'All';
        }
        if (event === 'In Stock') {
            this.selectedRadioBtn = 'true';
        }
        if (event === 'Out of Stock') {
            this.selectedRadioBtn = 'false';
        }
    };
    __decorate([
        core_1.Input()
    ], ProductListComponent.prototype, "searchTextValue");
    ProductListComponent = __decorate([
        core_1.Component({
            selector: 'product-list',
            standalone: true,
            imports: [common_1.CommonModule, product_component_1.ProductComponent, filter_component_1.FilterComponent],
